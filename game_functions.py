...
def check_keydown_events(event, ai_settings, screen, stats, scoreboard, ship,
                         aliens, bullets):
...
def check_events(ai_settings, screen, stats, scoreboard, play_button, ship,
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(event, ai_settings, screen, stats, scoreboard,
                                 ship, aliens, bullets)
...
def update_screen(ai_settings, screen, stats, scoreboard, ship, aliens, bullets,
                  play_button):
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    ...
    alien.rect.y = 2 * alien.rect.height + 30 + 2 * alien.rect.height * row_number
    aliens.add(alien)
...
def check_aliens_bottom(ai_settings, stats, screen, scoreboard, ship, aliens,
                        bullets):
    screen_rect = screen.get_rect()
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            ship_hit(ai_settings, stats, screen, scoreboard, ship, aliens,
                     bullets)
            break
...
def update_aliens(ai_settings, stats, screen, scoreboard, ship, aliens,
                  bullets):
    check_fleet_edges(ai_settings, aliens)
    check_aliens_bottom(ai_settings, stats, screen, scoreboard, ship, aliens,
                        bullets)
    aliens.update()
